...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const loadData = () => {
      ...
      const resourcesCount = savedResources ? JSON.parse(savedResources).length : 0;

      const savedNotices = localStorage.getItem('notices');
      const noticesList = savedNotices ? JSON.parse(savedNotices) : [];
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-8 py-10">
        <div className="flex justify-between items-end mb-10">
          ...
          <p className="text-gray-500">2025년 4월 5일 토요일</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <Link href="/schedule" className="group">
            <div className="bg-white rounded-3xl p-8 hover:shadow-xl transition-all border border-emerald-100 hover:border-emerald-300">
              ...
              <div className="mt-8">
                <p className="text-5xl font-bold text-gray-900">{dashboardData.schedules}</p>
                <p className="text-xl font-medium text-gray-700 mt-3">다가오는 일정</p>
                <p className="text-gray-500 mt-2 text-sm">이번 주 {dashboardData.schedules}개의 일정이 있어요</p>
              </div>
          ...
          <Link href="/attendance" className="group">
            <div className="bg-white rounded-3xl p-8 hover:shadow-xl transition-all border border-amber-100 hover:border-amber-300">
              ...
              <div className="mt-8">
                <p className="text-5xl font-bold text-amber-600">{dashboardData.attendanceRate}%</p>
          ...
          <Link href="/resources" className="group">
            <div className="bg-white rounded-3xl p-8 hover:shadow-xl transition-all border border-purple-100 hover:border-purple-300">
              ...
              <div className="mt-8">
                <p className="text-5xl font-bold text-gray-900">{dashboardData.resources}</p>
                <p className="text-xl font-medium text-gray-700 mt-3">자료실</p>
                <p className="text-gray-500 mt-2 text-sm">
                  {dashboardData.resources > 0 
                    ? `최신 자료가 업데이트됐어요` 
                    : '아직 등록된 자료가 없습니다'}
                </p>
              </div>
